...
sap.ui.define([
	...
	return Controller.extend("sap.ZG001.timesheet.input.weekly.controller.ZZmain", {
		...
		onTimePickerChange: function(aControlEvent) {
			...
			if (sId.substr(sId.length-12,12) !== "WorkingHours"){
				this.getView().byId(sId.substr(1,1)+"WorkingHoursE").fireChange();
			}
		...
		_timeCheck: function() {
			...
			var oSelf = this;
			var oWorkingHoursData = oSelf.getView().getModel("WorkingHours").getData();
			for(var iCount=0; iCount<oWorkingHoursData.length; iCount++){
				if( oWorkingHoursData[iCount]["FlgTarget"] === true ){
					oSelf.getView().byId(iCount+"WorkingHoursE").fireChange();
				}
			}
		},
...
//			});
			...
			for (var iTSCount = 0; iTSCount < oTimesheetData.getData().length; iTSCount++) {
				...
				for (var iTSDays = 0; iTSDays < 7; iTSDays++ ) {
					...
					var sTSPath = "/" + iTSDays + "/";
					if (oWorkingHours.getProperty(sTSPath + "FlgTarget")) {

						var oUITimesheetHours = oUITimesheetData[iTSCount].getAggregation("content")[0]
																		  .getAggregation("content")[0]
																		  .getAggregation("content")[iTSDays + 2]
																		  .getAggregation("content")[0]
																		  .getAggregation("items")[0];
						
						if( Number(sDayHoursArray[iTSDays]["WeekHours"]) % 0.25 !== 0){
							oUITimesheetHours.setValueStateText(oI18n.getText("ZF_MSG014"));
							oUITimesheetHours.setValueState("Error");
							oErrorArray.push(oUITimesheetHours);
						} else {
							oUITimesheetHours.setValueState("None");
						}
					}
