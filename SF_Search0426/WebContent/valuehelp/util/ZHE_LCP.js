...
//	var bIsInitialized = false;
	...
	let oZHE_LCP = BaseObject.extend("sap.pwaa.valuehelp.util.ZHE_LCP", {
...
//		bIsInitialized = true;
		...
		oTemplate = new sap.m.ColumnListItem({
			...
			cells : [  new sap.m.ObjectIdentifier({
				title : "{result>lcpcode}"
			}), new sap.m.ObjectIdentifier({
				title : "{result>lcpname}"
			}) ]
		...
		oValueHelpDialog = sap.ui.xmlfragment(
				"sap.pwaa.valuehelp.view.ZHE_LCP_VH", this);

		/* Create FilterBar */
		oFilterBar = sap.ui.getCore().byId("ZHE_LCP_FB");
...
//		 }));
		...
		oResultTable = sap.ui.xmlfragment(
				"sap.pwaa.valuehelp.view.ZHE_LCP_RT", this);
