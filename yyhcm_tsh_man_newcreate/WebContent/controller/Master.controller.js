...
sap.ui.define([
		...
		return BaseController.extend("soreturn.controller.Master", {
			...
			onOpenViewSettings : function () {
				if (!this._oViewSettingsDialog) {
					this._oViewSettingsDialog = sap.ui.xmlfragment("soreturn.view.ViewSettingsDialog", this);
			...
			onConfirmViewSettingsDialog : function (oEvent) {
				...
				aFilterItems.forEach(function (oItem) {
					switch (oItem.getKey()) {
						case "Filter1" :
							aFilters.push(new Filter("Kwmeng", FilterOperator.LE, 100));
							break;
						case "Filter2" :
							aFilters.push(new Filter("Kwmeng", FilterOperator.GT, 100));
							break;
						default :
							break;
					}
			...
			onMenuAction: function(oEvent) {
				...
				var sSelectedItem = oEvent.getParameter("item").getText();
				...
				switch(sSelectedItem)
				{
					...
					case "File Upload":
						...
						this.getRouter().navTo("upload", {}, false);
						break;
				}
			...
			_onMasterMatched :  function() {
				this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
					function (mParams) {
						...
						var sObject = mParams.firstListitem.getBindingContext().getProperty("Vbeln");
						var sItem	= mParams.firstListitem.getBindingContext().getProperty("Posnr");
						this.getRouter().navTo("object", { sObject , sItem }, true);
					}.bind(this),
					function (mParams) {
						if (mParams.error) {
							return;
			...
			_showDetail : function (oItem) {
				...
				var oObject = {
					sObject : oItem.getBindingContext().getProperty("Vbeln"),
					sItem   : oItem.getBindingContext().getProperty("Posnr")
				}
				this.getRouter().navTo("object", oObject, bReplace);
			},
